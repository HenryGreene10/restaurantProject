...
export const OnboardingPage: React.FC<{
    ...
    async function loadOnboardingState() {
      ...
      try {
        ...
        if (onboardingState.matched && onboardingState.tenantSlug) {
          setSuccessMessage("Found your restaurant workspace. Redirecting to the dashboard…")
          await onCompleted()
          return
        }
      } catch (error) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSuccessMessage("Restaurant created. Redirecting to your dashboard…")
      await onCompleted()
    } catch (error) {
    ...
    <main className="mx-auto flex min-h-[100dvh] w-full max-w-[960px] flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
      <header className="grid gap-3">
        ...
        <div className="grid gap-2">
          ...
          <p className="max-w-3xl text-base leading-7 text-muted-foreground">
            Set your restaurant name and choose the tenant slug your admin and storefront will use.
          </p>
        </div>
      </header>
      ...
      <Card className="border border-border/80 bg-card shadow-sm">
        <CardHeader className="gap-1">
          ...
          <CardDescription>
            Your Clerk account will become the owner account for this restaurant.
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <form className="grid gap-5" onSubmit={handleSubmit}>
            ...
            <div className="grid gap-2">
              ...
              />
              <p className="text-sm text-muted-foreground">
                This becomes your URL slug, for example <code>joes-pizza.easymenu.website</code>.
              </p>
              {isCheckingSlug ? (
                ...
                <p className={slugState.available ? "text-sm text-emerald-600" : "text-sm text-destructive"}>
                  {slugState.available
                    ? "Slug is available"
