...
export function CustomerOtpStep({
  ...
  return (
    <div className="space-y-6 px-1 py-2 sm:space-y-8">
      ...
      <div className="flex justify-center gap-1.5 sm:gap-2" onPaste={handlePaste}>
        {digits.map((digit, i) => (
          <input
            ...
            className="h-12 w-10 rounded-[var(--radius)] border border-input bg-background text-center text-lg font-semibold text-foreground shadow-sm outline-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/30 sm:h-14 sm:w-11 sm:text-xl"
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-col items-start gap-2 text-xs text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
          {countdown > 0 ? (
            <span>Resend code in 0:{String(countdown).padStart(2, '0')}</span>
          ) : (
            <button
              type="button"
